...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                successorUnexploredCorners = list(unexploredCorners)
                if (nextx,nexty) in unexploredCorners:                    
                    successorUnexploredCorners.remove((nextx,nexty))
                successors.append( ( ((nextx, nexty),successorUnexploredCorners), action, 1) )
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    node = state[0]
    unexporedCorners = list(state[1])
    sum = 0
    '''for corner in unexporedCorners:
        h_cost = 0
        for nextCorner in unexporedCorners:
            if h_cost == 0 or h_cost > util.manhattanDistance(node, nextCorner):
                h_cost = util.manhattanDistance(node, nextCorner)
                sum += h_cost
                corner = nextCorner
        node = nextCorner
        unexporedCorners.remove(nextCorner)'''
    
    while len(unexporedCorners) != 0:
        h_cost = 0
        for corner in unexporedCorners:
            if h_cost == 0 or h_cost > util.manhattanDistance(node, corner):
                h_cost = util.manhattanDistance(node, corner)
                nextCorner = corner
        sum += h_cost        
        node = nextCorner
        unexporedCorners.remove(nextCorner)
...
def foodHeuristic(state, problem):
    ...
    if len(foodPosition) != 0:
        for food in foodPosition:
            if h_cost == 0 or h_cost < util.manhattanDistance(node, food):
                h_cost = util.manhattanDistance(node, food)
    return h_cost
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        visited_node = []
        stack_node = util.Queue()
        stack_node.push((gameState.getPacmanPosition(),[]))
        "*** YOUR CODE HERE ***"
        nearestDistance = 999999
        for i in range(len(foodPosition)):
            distance = abs(gameState.getPacmanPosition()[0]-foodPosition[i][0]) + abs(gameState.getPacmanPosition()[1]-foodPosition[i][1])
        ...
        while not stack_node.isEmpty():
            node = stack_node.pop()
            result = node[1]
            goalState = foodPosition[index]
            for record in problem.getSuccessors(node[0]):
                leaf = record[0]
                action = record[1]
                sub_result = list(result)
                
                if leaf == foodPosition[index]:
                    result.append(action)
                    return result
            
                if leaf not in visited_node:
